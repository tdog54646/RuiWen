...
class RagIndexServiceWriteOrderTest {
    ...
    void failedAddDoesNotDeleteExistingChunksFirst() throws Exception {
        ...
        RagVectorStoreFactory vectorStoreFactory = mock(RagVectorStoreFactory.class);
        RagIndexManager indexManager = mock(RagIndexManager.class);
        ...
        when(indexManager.readAlias()).thenReturn("test-rag-index");
        when(vectorStoreFactory.forIndex("test-rag-index")).thenReturn(vectorStore);
        ...
        RagIndexService service = new RagIndexService(
                vectorStoreFactory, indexManager, mapper, storage, es, chunker);
